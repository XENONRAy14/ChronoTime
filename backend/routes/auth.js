...
router.post('/register', rateLimiter, async (req, res) => {
  try {
    const invalid = validate.credentials(req.body, true);
    ...
    res.status(error.code === 11000 ? 409 : 500).json({ message: error.code === 11000 ? 'Ce compte existe déjà.' : 'Opération impossible, veuillez réessayer.' });
  }
});
...
router.post('/login', rateLimiter, async (req, res) => {
  try {
    const invalid = validate.credentials(req.body);
    if (invalid) return res.status(400).json({ message: invalid });
