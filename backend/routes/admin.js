...
router.param('id', (req,res,next,id) => objectId(id) ? next() : res.status(400).json({message:'Identifiant invalide'}));
...
router.delete('/users/:id', adminAuth, async (req, res) => {
  try {
    ...
    if (String(req.user._id) === userId) return res.status(403).json({success:false,message:'Vous ne pouvez pas supprimer votre propre compte ici.'});
    ...
    if (user.isAdmin) return res.status(409).json({success:false,message:'Rétrogradez cet administrateur avant de le supprimer.'});
...
router.patch('/users/:id/demote', adminAuth, async (req, res) => {
  try {
    if (String(req.user._id) === req.params.id) return res.status(403).json({ success:false, message:'Un autre administrateur doit modifier votre rôle.' });
    const user = await User.findById(req.params.id);
    if (!user) {
      return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' });
    }
